...
- **Firebase Auth Web SDK:** Utilizza Firebase Auth per gestire l'accesso sicuro sia con account Google (`signInWithPopup` su desktop, con fallback automatico a `signInWithRedirect` su browser mobile/iOS) sia tramite e-mail e password (`signInWithEmailAndPassword`, `createUserWithEmailAndPassword`). Supporta l'accesso anonimo (`signInAnonymously`). I dati salvati come ospite sono memorizzati localmente e l'account anonimo può essere collegato ad un account Google per preservare i dati.
- **Stato di Sessione e Redirect:** Al bootstrap dell'app, `getRedirectResult(auth)` viene invocato per raccogliere l'esito dell'autenticazione tramite redirect, potendo restituire `null` se il caricamento non proviene da una redirezione attiva. Di conseguenza, il listener asincrono globale `onAuthStateChanged` rimane l'unica fonte di verità primaria ed affidabile per confermare la sessione utente attiva e lo stato del profilo.
- **Limitazioni iOS/Safari:** Su iOS Safari (specialmente in modalità PWA standalone), `signInWithRedirect` è gestito come *best effort* a causa delle limitazioni del browser nell'isolare il contesto delle webview standalone dall'app Safari principale, potendo richiedere accessi multipli.
- **Password Reset:** È presente un flusso di recupero password tramite e-mail (`sendPasswordResetEmail`) integrato direttamente nella schermata di accesso e-mail.
