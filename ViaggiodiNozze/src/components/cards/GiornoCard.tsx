...
export default function GiornoCard({
  ...
  return (
    <div 
      className={"rounded-2xl border transition-all p-4.5 bg-slate-900/90 " + (isSelected ? 'border-amber-500/60 shadow-lg shadow-amber-500/5' : 'border-slate-800 hover:border-slate-700/80')}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 cursor-pointer" onClick={onSelect}>
          ...
          {day.notes && (
            <p className="text-xs text-slate-400/90 mt-2 p-2 rounded-lg bg-slate-800/60 border border-slate-800/80 italic">
              {day.notes}
            </p>
          )}
        </div>
      ...
      <div className="mt-3 pt-3 border-t border-slate-800 flex items-center justify-between">
        ...
        <button
          ...
          className="text-xs font-medium text-slate-300 hover:text-white px-2.5 py-1 rounded-md bg-slate-800 hover:bg-slate-700/80 transition-colors flex items-center gap-1 cursor-pointer"
        >
