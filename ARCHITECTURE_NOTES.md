...
- **Firebase Auth Web SDK:** Utilizza Firebase Auth per gestire l'accesso sicuro con Google (`signInWithPopup` su desktop, con fallback automatico a `signInWithRedirect` su browser mobile/iOS) e l'accesso anonimo (`signInAnonymously`). I dati salvati come ospite sono memorizzati localmente e l'account anonimo può essere collegato ad un account Google per preservare i dati.
